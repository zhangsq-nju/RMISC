...
def _filter_index_rows(indexes, prediction_length):
    filtered = []
    for row in indexes:
        try:
            if int(row[3]) - int(row[2]) >= prediction_length:
                filtered.append(row)
...
def load_data(data, context_len=128, uni=False, prediction_length=64):
    roots = [x.strip() for x in str(data).split('+') if x.strip()]
    ...
    for root in roots:
        train_files = glob.glob(f"{root}/DataIndex/train/*.csv")
        val_files = glob.glob(f"{root}/DataIndex/val/*.csv")
        test_files = glob.glob(f"{root}/DataIndex/test/*.csv")

        for file in train_files:
            print(file)
            indexes = _filter_index_rows(pd.read_csv(file).values.tolist(), prediction_length)
            # [root, dataset, parquet, start, stop, target, cov]
            train_list.extend([[root] + row for row in indexes])

        for file in val_files:
            print(file)
            indexes = _filter_index_rows(pd.read_csv(file).values.tolist(), prediction_length)
            val_list.extend([[root] + row for row in indexes])

        for file in test_files:
            print(file)
            indexes = _filter_index_rows(pd.read_csv(file).values.tolist(), prediction_length)
            test_list.extend([[root] + row for row in indexes])
