...
class TimesFm2_5Model(TimesFm2_5PreTrainedModel):
    ...
    def _revin(
        ...
        loc = torch.nan_to_num(loc.to(hidden_states.device), nan=0.0, posinf=0.0, neginf=0.0).detach()
        scale = torch.nan_to_num(scale.to(hidden_states.device), nan=1.0, posinf=1.0, neginf=1.0).detach()
        safe_scale = torch.clamp(scale, min=self.tolerance)

        if reverse:
            return hidden_states * safe_scale + loc
    ...
    def _update_running_stats(
        ...
        """
        new_values = torch.nan_to_num(new_values, nan=0.0, posinf=0.0, neginf=0.0)
        eps = torch.finfo(new_values.dtype).eps
        ...
        inc_std = torch.sqrt(torch.clamp(inc_var, min=eps))
        inc_std = torch.where(inc_count == 0, torch.zeros_like(inc_std), inc_std)
        ...
        new_std = torch.sqrt(torch.clamp(new_var, min=eps))
        new_std = torch.where(new_count == 0, torch.zeros_like(new_std), new_std)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        patch_padding_expanded = patch_padding.unsqueeze(-1)
        input_embeddings = torch.where(
            patch_padding_expanded,
            torch.zeros_like(input_embeddings),
            input_embeddings,
        )
...
class TimesFm2_5ModelForPrediction(TimesFmModelForPrediction):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        input_mins = []
        for ts in inputs:
            finite_ts = ts[torch.isfinite(ts)]
            if finite_ts.numel() > 0:
                input_mins.append(torch.min(finite_ts))
            else:
                input_mins.append(torch.zeros((), dtype=ts.dtype, device=ts.device))
        input_min = torch.min(torch.stack(input_mins))
        ...
        input_padding_bool = input_padding[:, : input_ts.shape[1]] >= 0.5
        finite_inputs = torch.isfinite(input_ts)
        valid_inputs = (~input_padding_bool & finite_inputs).to(input_ts.dtype)
        clean_input_ts = torch.nan_to_num(input_ts, nan=0.0, posinf=0.0, neginf=0.0)
        valid_count = valid_inputs.sum(dim=1, keepdim=True)
        safe_count = valid_count.clamp_min(1.0)
        mu_global = (clean_input_ts * valid_inputs).sum(dim=1, keepdim=True) / safe_count
        centered = (clean_input_ts - mu_global) * valid_inputs
        sigma_global = torch.sqrt(torch.clamp((centered.square()).sum(dim=1, keepdim=True) / safe_count, min=0.0))
        mu_global = torch.where(valid_count > 0, mu_global, torch.zeros_like(mu_global)).detach()
        sigma_global = torch.where(valid_count > 0, sigma_global, torch.ones_like(sigma_global)).detach()
        sigma_global = torch.clamp(sigma_global, min=self.model.tolerance)

        normalized_ts = self.model._revin(
            clean_input_ts,
            mu_global,
            sigma_global,
            reverse=False,
            mask=input_padding_bool,
        )
