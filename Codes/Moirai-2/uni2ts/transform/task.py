...
@dataclass
class MaskedPrediction(MapFuncMixin, CheckArrNDimMixin, Transformation):
    prediction_length: int | None = None  ### 修改了prediction_length，允许由yaml传入固定预测长度
    target_field: str = "target"
    truncate_fields: tuple[str, ...] = tuple()
    optional_truncate_fields: tuple[str, ...] = tuple()
    prediction_mask_field: str = "prediction_mask"
    ...
    def __post_init__(self):
        assert self.prediction_length > 0, "prediction_length must be positive"

    def __call__(self, data_entry: dict[str, Any]) -> dict[str, Any]:
        target = data_entry[self.target_field]
        ### 加入了patch_size读取，固定prediction_length时用于换算patch token数
        patch_size = data_entry.get("patch_size")
        prediction_mask = self._generate_prediction_mask(target, patch_size)
        self.map_func(
            partial(self._truncate, mask=prediction_mask),  # noqa
            data_entry,
            self.truncate_fields,
    ...
    def _generate_prediction_mask(
        ...
        patch_size: int | np.integer | None = None,
    ) -> Bool[np.ndarray, "var time"]:
        self.check_ndim("target", target, self.expected_ndim)
        var, time = target.shape[:2]
        prediction_mask = np.zeros((var, time), dtype=bool)
        if patch_size is None:
            raise KeyError(
                "patch_size is required when MaskedPrediction.prediction_length is set"
            )
        ### 加入了patch_size标量化，兼容numpy标量或数组
        patch_size = int(np.asarray(patch_size).reshape(-1)[0])
        ### 修改了mask长度，由yaml prediction_length换算成patch token数
        mask_length = int(np.ceil(self.prediction_length / patch_size))
        ### 加入了边界保护，避免mask长度超过当前token长度
        mask_length = min(time, max(1, mask_length))
        prediction_mask[:, -mask_length:] = True
        return prediction_mask
