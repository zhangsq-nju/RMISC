...
class PatchCrop(MapFuncMixin, Transformation):
    """
    Deterministically crop fields in a data_entry.

    No random crop.
    No random offset.
    Assumes the sequence length has already been made compatible with patch_size
    ...
    def _get_boundaries(self, data_entry: dict[str, Any]) -> tuple[int, int]:
        ...
        max_time_patches = self.max_patches // nvar

        max_length = max_time_patches * patch_size

        start = 0
        stop = min(time, max_length)
