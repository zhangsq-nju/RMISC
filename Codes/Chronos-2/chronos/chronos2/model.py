...
class Chronos2Model(PreTrainedModel):
    ...
    def __init__(self,  config: Chronos2CoreConfig):
        ...
        self.input_patch_embedding = ResidualBlock(
            in_dim=self.chronos_config.input_patch_size,
            h_dim=config.d_ff,
        ...
        self.batch_position_encoding = nn.Embedding(24, config.d_model)
    ...
    def _init_weights(self, module):
        ...
        if module is self.batch_position_encoding:
            nn.init.trunc_normal_(module.weight, mean=0.0, std=factor * (self.config.d_model ** -0.5))
    ...
    def build_position_ids(self,group_ids: torch.Tensor):
        assert group_ids.dim() == 1
        group_ids = group_ids.unsqueeze(0)  # -> (1, T)

        B, T = group_ids.shape

        is_start = torch.zeros_like(group_ids, dtype=torch.bool)
        is_start[:, 0] = True
        is_start[:, 1:] = group_ids[:, 1:] != group_ids[:, :-1]

        arange = torch.arange(T, device=group_ids.device).unsqueeze(0).expand(B, T)

        group_start_pos = torch.zeros_like(group_ids)
        group_start_pos[is_start] = arange[is_start]
        group_start_pos = torch.cummax(group_start_pos, dim=-1).values

        position_ids = arange - group_start_pos
    ...
    def _prepare_patched_future(
        ...
        patched_future = patched_future_covariates
    ...
    def encode(
        ...
        batch_position_ids = self.build_position_ids(group_ids)
        input_embeds += self.batch_position_encoding(batch_position_ids.unsqueeze(1).expand(-1, input_embeds.shape[-2]))
    ...
    def forward(
        ...
        # Get MAE Loss
        """
        preds=quantile_preds.median(dim=1).values
        val_loss=nn.L1Loss()
        future_target, _ = self.instance_norm(future_target, loc_scale)
        loss_mask = ~torch.isnan(future_target)
        _,l=future_target.shape
        future_target = future_target[loss_mask]
        preds = preds[:, :l][loss_mask]
        loss=val_loss(preds,future_target)
        """
        quantile_preds = rearrange(
            quantile_preds,
            "b q h -> b (q h)",
